  ...
  return (
    <div className="w-full h-full p-6 flex flex-col">
      <div className="mb-4 flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Live Location Map</h2>
          <p className="text-slate-400 text-sm">Real-time triangulation visualization (Simulated)</p>
        </div>
      ...
      <div className="flex-1 bg-slate-950 rounded-xl border border-slate-800 relative overflow-hidden shadow-2xl shadow-black/50">
        ...
        {devices.map((device) => (
          ...
          >
            ...
            {/* Tooltip */}
            <div className="absolute top-6 left-1/2 transform -translate-x-1/2 w-48 bg-slate-800 text-xs rounded shadow-lg p-2 opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none border border-slate-700">
              <div className="font-bold text-white mb-1">{device.name}</div>
              <div className="text-slate-400 font-mono text-[10px]">{device.macAddress}</div>
